    ...
    BEGIN
			...
			WHILE @@FETCH_STATUS = 0
				BEGIN	
					...
					IF (@Card3 IS NOT NULL) AND (@Card2 IS NOT NULL) AND (@Card1 IS NOT NULL)					 			
						AND (@Card3 != @Card2) AND (@Card2 != @Card1) 
						BEGIN
					...
					IF NOT EXISTS (SELECT * FROM AutoZiczacF WHERE ID = @ID)		 
						INSERT INTO AutoZiczacF(ID, Card, Predict, Profit, ProfitMod, Coeff, UIResult, AutoSessionID, Profit_CutLoss, ProfitMod_CutLoss) 
						VALUES
